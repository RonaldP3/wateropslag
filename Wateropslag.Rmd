...
Voor weerstation `r renderText({toString(input$station)})` is het verschil tussen een gemiddeld neerslagjaar en het droogste scenario **`r renderText({round(verschil(), 1)})`** milimeter. Bij een tuinoppervlak van `r renderText({input$tuin})` m^2^ is een opslag van **`r renderText({round(opslag(), 0)})`** liter nodig om het neerslagverschil te overbruggen.
